...
if (isset($_POST['update'])) {
    ...
    // }
        ...
        try {
            ...
            $stmt = $con->prepare("UPDATE `temp_hotel` SET `name`=?, `description`=?, `price`=?, `hotel_location`=?,`room`=?, `updated_at`=? WHERE email=? ");
            $stmt->bind_param("ssssssss", $submitted_name, $submitted_description, $submitted_price, $submitted_location, $submitted_room, $updated_at, $email);
            $stmt->execute();
            $stmt->close();

            // Update hotels table
            $stmt2 = $con->prepare("UPDATE `hotels` SET `name`=?, `description`=?, `price`=?, `location`=?,`room`=? WHERE email=? ");
            $stmt2->bind_param("sssssss", $submitted_name, $submitted_description, $submitted_price, $submitted_location, $submitted_room, $email);
            $stmt2->execute();
            $stmt2->close();

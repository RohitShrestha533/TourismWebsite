            ...
            if(mysqli_num_rows($query1)==1)
            {
                ...
                try {
                    ...
                    if (!empty($filePaths)) {
                        ...
                        $insertQuery = mysqli_query($con, "INSERT INTO `temp_package`(`name`, `hotel_id`, `description`, `price`, `location`, `packstatus`, `image`, `duration`, `map`, `action`, `email`, `created_at`)
                        VALUES ('$name', '$hotel_id', '$description', '$price', '$location', '$status', '$combinedFilePaths', '$duration', '$map', '$action', '$email', '$create_date')");
                        $insertQuery2 = mysqli_query($con, "INSERT INTO `notification`(`nFrom`, `ntype`, `message`, `date`)
                         VALUES ('$name','Packages','Request For Adding New Package FROM $hname ','$create_date')");
